                ...
                $(document).ready(function () {
                    ...
                    $.get("getAllUsers", null, function (data) {
                        for (var i = 0; i < data.length; i++) {
                            $("#users").append("<div class='row' id='row" + data[i]["id"] + "'><div class='col'>" + data[i]["username"] + "</div><div class='col'><button id='changepass-btn' name='userID' value='" + data[i]["id"] + "'class='btn btn-sm btn-outline-dark' onclick='showDiv()'>Change Password</button></div></div><br>");
                        }
                    });
                $.get("getUserNotifications", null, function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        $("#notifications").append("<tr><td><a href='getNotification?ID=" + data[i]["notificationID"] + "'>"
                                + data[i]["notification"] + "<br>" + data[i]["notificationDate"] + "</a></td></tr>");
